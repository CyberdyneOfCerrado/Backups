...
public class BancoDeDados 
{
	...
	final Tabela regraBackup = new Tabela("REGRABACKUP","CREATE TABLE REGRABACKUP ( PRIMARYKEY INT NOT NULL GENERATED ALWAYS AS IDENTITY(START WITH 1, INCREMENT BY 1), DESTINO VARCHAR(1000), ORIGEM VARCHAR(10000), FK INT NOT NULL, PRIMARY KEY (PRIMARYKEY), CONSTRAINT fk_REGRABACKUP_BACKUP1 FOREIGN KEY (FK) REFERENCES BACKUP (PRIMARYKEY) )");
	final Tabela versao      = new Tabela("VERSAO","CREATE TABLE VERSAO (PRIMARYKEY INT NOT NULL GENERATED ALWAYS AS IDENTITY(START WITH 1, INCREMENT BY 1),DATAINICIO VARCHAR(100) ,STATUS VARCHAR(100) ,FK INT NOT NULL,PRIMARY KEY (PRIMARYKEY),CONSTRAINT fk_VERSAO_BACKUP1 FOREIGN KEY (FK)REFERENCES BACKUP (PRIMARYKEY))");
	final Tabela artefato    = new Tabela("ARTEFATO","CREATE TABLE ARTEFATO(PRIMARYKEY INT NOT NULL GENERATED ALWAYS AS IDENTITY(START WITH 1, INCREMENT BY 1),NOME VARCHAR(1000), ULTIMAMODIFICACAO VARCHAR(100) ,FK INT NOT NULL,PRIMARY KEY (PRIMARYKEY),CONSTRAINT fk_ARTEFATO_VERSAO1 FOREIGN KEY (FK) REFERENCES VERSAO(PRIMARYKEY))");
	final Tabela dias        = new Tabela("DIAS","CREATE TABLE DIAS ( PRIMARYKEY INT NOT NULL GENERATED ALWAYS AS IDENTITY(START WITH 1, INCREMENT BY 1),DIA VARCHAR(100) ,HORAS CHAR(100) ,DESLIGAR VARCHAR(100),FK INT NOT NULL,PRIMARY KEY (PRIMARYKEY),CONSTRAINT fk_DIAS_REGRABACKUP FOREIGN KEY (FK) REFERENCES REGRABACKUP (PRIMARYKEY))");
	//Fim definição.
